...
from dataset.transform_wrapper import TRANSFORMS
from torchvision.transforms import Compose, RandomResizedCrop, ToTensor, ToPILImage, CenterCrop, Resize, Normalize, RandomHorizontalFlip
import PIL
import numpy as np
...
class BaseSet(Dataset):
    def __init__(self, mode="train", cfg=None, transform=None):
        ...
        self.cfg = cfg
        self.input_size = cfg.INPUT_SIZE # (224, 224)
        self.data_type = 'jpg' # jpg
        self.color_space = 'RGB' # RGB
        self.size = self.input_size
        self.dual_sample = True
        self.data_root = cfg.data_root
        print("Use {} Mode to train network".format(self.color_space))

        if self.mode == "train":
            print("Loading train data ...", end=" ")
            self.json_path = cfg.data_root + '/full_VQA_Med_2020_train.json'
        elif "val" in self.mode:
            print("Loading valid data ...", end=" ")
            self.json_path = cfg.data_root + '/full_VQA_Med_2020_val.json'
        else:
    ...
    def __getitem__(self, index):
        ...
        img = self._get_image(now_info) # 获取图像
        meta = dict()
    ...
    def update_transform(self, input_size=None):
        ...
        # )
        transform_ops = (RandomResizedCrop(size=self.cfg.INPUT_SIZE, scale=(0.08, 1.0), ratio=(0.75, 1.3333),
                                           interpolation=PIL.Image.BILINEAR),
                         RandomHorizontalFlip(p=0.5)) \
            if self.mode == 'train' else \
            (Resize(size=256, interpolation=PIL.Image.BILINEAR), CenterCrop(size=(224, 224)))
    ...
    def _get_class_dict(self):
        ...
        for i, anno in enumerate(self.data):
            ...
            if not cat_id in class_dict:
                class_dict[cat_id] = []
            class_dict[cat_id].append(i)  # category_id作为key, value为序号i的list
        return class_dict
